...
import { birthAPI, deathAPI, reportAPI } from '../../../services/api';
...
const HospitalStatistiques = () => {
  ...
  // ✅ Envoyer les statistiques aux autorités (SANS vérification abonnement)
  const handleSendStats = async () => {
    ...
    try {
      ...
      console.log("📤 [handleSendStats] Données envoyées:", reportData);
      
      const response = await reportAPI.createReport(reportData);
      console.log("✅ [handleSendStats] Réponse:", response.data);
      
      setSendSuccess(true);
      setTimeout(() => setSendSuccess(false), 5000);
    } catch (err) {
      console.error('❌ [handleSendStats] Erreur complète:', err);
      console.error('❌ [handleSendStats] Réponse erreur:', err.response?.data);
      
      const errorData = err.response?.data;
      const errorDetail = errorData?.detail || errorData?.errors || JSON.stringify(errorData) || 'Erreur inconnue';
      
      if (errorData?.code === 'REPORT_EXISTS') {
        setError(t('hospital.statistiques.reportExists'));
      } else {
        setError(`${t('hospital.statistiques.sendError')} — ${errorDetail}`);
      }
    } finally {
      setSending(false);
    }
    ...
    <div className={`space-y-6 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-slide-in-from-top">
        ...
        <div className="flex gap-3">
          ...
          {/* ✅ BOUTON ENVOYER AUX AUTORITÉS (toujours actif) */}
          <button
            onClick={handleSendStats}
            disabled={sending}
            className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl font-medium hover:shadow-lg transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
          >
            {sending ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
